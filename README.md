...
*/5 * * * * pgrep -f "gunicorn -w 4 -b gcpnet-ema.missouri.edu:80 --timeout 300 src.wsgi:app" || cd /bml/acmwhb/Repositories/Lab_Repositories/GCPNet-EMA && mamba run -n GCPNet-EMA "gunicorn -w 4 -b gcpnet-ema.missouri.edu:80 --timeout 300 src.wsgi:app"
```

**NOTE**: You should substitute the `/bml/acmwhb/Repositories/Lab_Repositories/GCPNet-EMA` reference above with the absolute path to your personal copy of the repository.
