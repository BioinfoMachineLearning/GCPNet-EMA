...
@beartype
def _edge_features(
    coords: Float[torch.Tensor, "num_nodes 3"],
    edge_index: Int64[torch.Tensor, "2 num_edges"],
    scalar_edge_feats: Float[torch.Tensor, "num_edges num_edge_scalar_features"],
    D_max: float = 4.5,
    num_rbf: int = 16,
    device: Union[torch.device, str] = "cpu"
) -> Tuple[
    Float[torch.Tensor, "num_edges num_cat_edge_scalar_features"],
    Float[torch.Tensor, "num_edges num_edge_vector_features", 3]
]:
...
def _node_features(
    coords: Float[torch.Tensor, "num_nodes 3"]
) -> Float[torch.Tensor, "num_nodes num_cat_node_vector_features 3"]:
        ...
        if protein_data is None or force_process_data:
            ...
            for protein_chain_id in protein_chain_ids:
                ...
                protein_atom_types.append(
                    torch.tensor(
                        [[ATOM_TYPES.index(a) for a in RES_ATOM14[EMADataset._safe_index_of_value(ALPHABET, s)]]
                ...
                protein_atom_chain_indices.append(
                    torch.tensor(
                        [[protein_chain_index for _ in RES_ATOM14[EMADataset._safe_index_of_value(ALPHABET, s)]]
                         for s in protein_chain_sequence]
                    )
                )
